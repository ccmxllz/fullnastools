...
class DbHelper:
    ...
    @DbPersist(_db)
    def insert_config_site(self, name, site_pri, rssurl, signurl, cookie, note, rss_uses,apikey,token):
        """
        插入站点信息
        """
        if not name:
    ...
    @DbPersist(_db)
    def update_config_site(self, tid, name, site_pri, rssurl, signurl, cookie, note, rss_uses,apikey,token):
        """
        更新站点信息
        """
        if not tid:
